...
let selectorDataArea = function() {
  request.post({ uri: backendUrl + "/vehicle/park", form: {} }, function(
    err,
    response,
    body
  ) {
...
function compareCoords(userC, areaC) {
  ...
  let a =
    ...
    Math.cos(userC.lat * (Math.PI / 180)) *
      Math.cos(areaC.lat * (Math.PI / 180)) *
      Math.sin(degLong / 2) *
      Math.sin(degLong / 2);
      ...
      if (document.getElementById("toggle_park")) {
        if (!navigator.geolocation) {
          ...
          watcher.id = navigator.geolocation.watchPosition(function(position) {
            ...
            if (tracker > 20) {
              that.setGPSMessage(
                "You are " + tracker + "km away from your parked vehicle"
              );
            }
          });
